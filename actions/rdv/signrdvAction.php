...
if(isset($_POST['validate'])){
    ...
    if(!empty($_POST['jourrdv']) AND !empty($_POST['Thoraire'])) {
        ...
        if ($checkAllAppoinmentAlreadyExistADay -> rowCount () < 5) {
            ...
            $checkAllAppoinmentAlreadyExistATimeSlot = $bdd -> prepare ('SELECT  *FROM rdv WHERE  Thoraire= ?');
            $checkAllAppoinmentAlreadyExistATimeSlot -> execute (array ($tranchehoraire1));
